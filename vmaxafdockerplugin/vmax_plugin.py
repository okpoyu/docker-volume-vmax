...
import six
from PyU4V import rest_univmax
...
class VmaxAf:
    ...
    def __init__(self, u4v_ip=None, user=None, password=None, port=8443,
        ...
        self.REST = rest_univmax.RestFunctions(
    ...
    def remove_volume(self, volume_name, volume_id):
        ...
        if volume_info['num_of_storage_groups'] > 0:
            for sg_id in volume_info['storageGroupId']:
                ...
                self.REST.remove_vol_from_sg(sg_id, volume_id)
        ...
        except exception.VMAXPluginException:
            is_volume_deleted = True
        return is_volume_deleted

    def find_ips(self, port_group):
        ips = []
        ports = self.REST.get_port_ids(port_group)
        for port in ports:
            LOG.debug(port)
    ...
    def create_volume(
        ...
        """
        is_valid_slo, is_valid_workload = self.REST.verify_slo_workload(
        ...
        storagegroup_name = self.REST.get_or_create_default_storage_group(
            volume_opts[SRP], volume_opts[SLO],
            volume_opts[WORKLOAD])
        try:
            ...
            raise exception.VMAXPluginException(exception_message)
        volume_info = self.REST.get_volume(volume_dict['device_id'])
    ...
    def attach_volume(self, volume_name, device_id, group_conf):
        ...
        default_sg_name = self.REST.get_default_storage_group_name(
    ...
    def get_or_create_masking_view(self, masking_view_dict, default_sg_name):
        ...
        masking_view_details = self.REST.get_masking_view(masking_view_name)
    ...
    def _create_new_masking_view(
        ...
        portgroup = self.REST.get_portgroup(port_group_name)
        ...
        error_message = (self.REST.create_masking_view(
            masking_view_name, storagegroup_name, port_group_name,
            init_group_name))
    ...
    def _get_or_create_initiator_group(self, init_group_name, connector):
        ...
        if found_init_group is None:
            self.REST.create_initiator_group(init_group_name, initiator_names)
    ...
    def _move_vol_from_default_sg(
        ...
        if check_vol:
            ...
            if num_vol_in_sg == 1:
                ...
                self.REST.delete_storage_group(default_sg_name)
    ...
    def _check_adding_volume_to_storage_group(
        ...
        if self.REST.is_volume_in_storagegroup(device_id, storagegroup_name):
            ...
            try:
                self.REST.add_volume_to_storage_group(
    ...
    def _validate_existing_masking_view(
        ...
        if not msg:
            ...
            portgroup = self.REST.get_portgroup(portgroup_name)
            if portgroup is None:
                msg = ("Cannot get port group: %(portgroup)s from the array "
                       "%(array)s. Portgroups must be pre-configured - please "
                       "check the array."
    ...
    def _delete_cascaded_storage_groups(self, child_sg_name, parent_sg_name,
        ...
        if child_sg_name != parent_sg_name:
            self.REST.delete_storage_group(parent_sg_name)
            LOG.debug("Storage Group %(storagegroup_name)s "
                      "successfully deleted.",
                      {'storagegroup_name': parent_sg_name})
        self.REST.delete_storage_group(child_sg_name)
            ...
            if initiator_group_name == default_ig_name:
                ...
                if not masking_view_names:
                    ...
                    if ig_details:
                        ...
                        self.REST.delete_initiator_group(initiator_group_name)
                else:
    ...
    def _last_volume_delete_masking_view(self, masking_view):
        ...
        self.REST.delete_masking_view(masking_view)
